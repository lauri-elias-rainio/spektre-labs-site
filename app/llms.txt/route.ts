...
export function GET() {
  ...
  const body = [
    ...
    `- Reserve + supply cross-check (runs live in your browser): ${absoluteUrl("/token")}`,
    "- Bitcoin reserve address: bc1qea8hasdf3mls29lmmxexvjv32ks22uzkjlgde9 — verify balance at mempool.space or blockstream.info",
    "- Solana token supply: query mint AaRuUWZ3EozpwAmbPtFtkcxK1Q9JSSmkKSFivnwaopAv via getTokenSupply on any Solana RPC",
    "- Pending (not yet published): BIP-322 signed message from reserve key; mint authority renouncement",
    "",
    "## Research Field Notes",
    ...articles.map(
      (article) =>
        `- ${article.title}: ${absoluteUrl(`/research/${article.slug}`)} — ${article.summary}`
    ),
    "",
    "## Primary Artifact",
